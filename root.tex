...
However, unlike Section \ref{SectionPerformance}, we simulate the camera measurements by creating artificial point-features and computing the bearings ($\mu_j^i$ in Equation (\label{eq:final1})) from the MAV to these points at a given frame rate.
By simulating the camera measurements this way, we can compare our estimations of the distances to the features ($\lambda_j^i$ in Equation (\label{eq:final1})) with the ground truth.
...
When using a real high-frequency camera, we can force the frame rate at 10Hz by discarding most of the provided frames.
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth, trim={0 2cm 0 4cm}, clip]{images/virtualExperiment.png}
...
For our evaluation, we consider a MAV flying in a room equipped with a \textbf{motion-capture system}.
...
The feature extraction and matching is done with FAST corner algorithm \cite{Rosten2006}.
