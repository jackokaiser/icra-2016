...
\begin{abstract}
Abstract1\\
State of the art approaches for visual-inertial sensor fusion use filter-based algorithms.
  ...
  Recently, this sensor fusion problem has also been addressed by using optimization-based approaches. These methods outperform filter-based algorithms in terms of accuracy due to their capability of relinearize past states. On the other hand, also in this case, having an automatic method able to quickly initialize the state before performing the optimization can be an advantage since it makes the optimization process more robust against the presence of local minima.
This paper introduces a new method to determine in closed-form a state initialization. Specifically, the proposed method determines the velocity and the attitude (roll and pitch angles) of a monocular camera by only using inertial
measurements and bearing measurements of several point features delivered by the camera in a short time interval. Additionally, also the distances of the observed features are determined. With respect to the method introduced in \cite{Martinelli2014}, which determines in closed-form the same physical quantities, the one here proposed is robust against biased gyroscope measurements.
The paper starts by analyzing the limitations of the method introduced in \cite{Martinelli2014}. This analysis, performed by making use of both real and synthetic data, clearly shows that the gyroscope bias significantly affects the performance of this method. The paper introduces a new method able to automatically estimate this bias. Compared to the original method, the new method is robust to this bias and simultaneously calibrates the gyroscope. We successfully evaluate the performance of the proposed method on both simulated and real data.
...
Abstract2\\
  ...
  In this paper, we study the impact of noisy sensors on the performance of this solution.
  ...
  Compared to the original method, the new method is now robust to this bias and automatically calibrates the gyroscope. We successfully evaluate the performance of the proposed approach on both simulated and real data.
\end{abstract}
...
A closed-form solution was provided in \cite{Engel2013} to determine the absolute scale.
...
However, their method also requires an altimeter to initialize the absolute scale.
...
\begin{figure}
  ...
  \caption{Test setup for identifying the performance limitations.
...
\begin{figure}
  \centering
    \resizebox{\columnwidth}{!}{\input{graph/droneMotion14.6for5s}}
    \caption{Motion performed by the drone in 5 seconds.}
\end{figure}

\begin{figure}
  \centering
    \resizebox{\columnwidth}{!}{\input{graph/originalCF}}
...
\begin{figure}
  \centering
  \resizebox{\columnwidth}{!}{\input{graph/biasEstimation12Ft}}
...
\begin{figure}
   \centering
   \resizebox{\columnwidth}{!}{\input{graph/CF3vsOpt12Ft}}
...
However, for very short time of integration ($<1.7$ seconds), the proposed method can fail by providing a gyroscope bias much larger than the true.
...
We validated this method by comparing its performance against the original method and the SVO described in \cite{Majdik2015} which is the-state-of-the-art approach for pose estimation on MAV.
