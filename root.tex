...
Specifically, the ground truth pose of the MAV is obtained with a motion-capture system.
...
\begin{figure}
  \centering
  \resizebox{0.7\columnwidth}{!}{\input{graph/droneMotion14.6for5s}}
...
Fig. \ref{fig:gBiasEstimate} displays the three components of the bias estimated by this minimization.
...
Since a MAV normally operates in near-hover confitions, $\vec{u}$ is approximated to the vector pointing upwards in the gyroscope frame when the MAV is standing.
...
Specifically, we can add a regularization term that penalizes high estimations of this gyroscope bias component:
...
we corrupt the gyroscope measurements with an artificial bias in Fig. \ref{fig:validBias}.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.489\columnwidth}
    ...
    \caption{$|bias|=0.1$.}
  \end{subfigure}
  \caption{Estimation error of the optimized closed-form solution against the original closed-form solution \cite{Martinelli2014}.
    The gyroscope measurements are corrupted with an artifial bias.
    The duration of integration is set to 2.8 seconds, and 10 point features are observed throughout the whole operation.\label{fig:validBias}}
\end{figure}

As seen in Fig. \ref{fig:validBias}, our method is robust against gyroscope bias whereas the original closed-form is not.
