...
We represent this setup in Figure \ref{fig:testsetup}.
...
in Figure \ref{fig:original}, we represent the quality of the evaluations with and without this additional constraint.
...
we corrupt the accelerometer measurements provided by our terrain IMU by adding an artificial bias (Figure \ref{fig:biasAccCF}).
...
As seen on the Figure \ref{fig:biasAccCF}, neither the gravity, the velocity or the lambdas are impacted by the accelerometer bias.
...
we corrupt the gyroscope measurements provided by our terrain IMU by adding an artificial bias (Figure \ref{fig:biasGyroCF}).
...
As seen in Figure \ref{fig:biasGyroCF}, the performance becomes very poor in presence of a bias on the gyroscope and, in practice, the overall method could only be successfully used with a very precise---and expensive---gyroscope.
...
\subsection{Non-linear minimization of the residual}
...
\begin{figure}[h!]
  ...
  \caption{Gyroscope bias estimation from non-linear minimization of the residual. We are observing 30 features  over a variable duration of integration. The true gyroscope bias is $[0.0276,   -0.0024,    0.0417]$. \label{fig:gBiasEstimate}}
\end{figure}
...
The optimized closed-form solution provides better results than the standard closed-form solution. Figure \ref{fig:optEstimate} depicts an improvement in precision of around $5\%$ for the distance to the features, and around $13\%$ for the speed after 4 seconds of integration.
...
Figure \ref{fig:biasGyroOpt} represents the quality of the estimations with the same artificial gyroscope bias from Figure \ref{fig:biasGyroCF}.



As seen in Figure \ref{fig:biasGyroOpt}, after a certain integration duration, the estimations agree no matter how high the bias is.
Moreover, the values on which these evaluations agree are equals to the ones in Figure \ref{fig:biasGyroCF} for the curve that has a gyroscope bias set to $0$.
