...
The method introduced in \cite{Martinelli2012}\cite{Martinelli2014} was only described in theory and demonstrated with Monte-Carlo simulations.
In this paper, we perform simulations with plausible MAV motions and synthetic noisy sensor data.
This allow us to identify limitations of the method and bring modifications to overcome them.
...
This is obtained by running the closed-form solution on plausible MAV motions with synthetic noisy sensor data.

In particular, we will consider the case of biased inertial measurements (both the accelerometers and the gyroscopes).
...
Synthetic gyroscope and accelerometer data are affected by a statistical error of $0.5$ deg/s and $0.5$ cm/s\textsuperscript{2}, respectively.
We will corrupt these measurements with a synthetic bias.

We set $7$ simulated 3D point-features about $3$m away from the MAV, which flies at a speed of around $1$m/s.
...
\subsection{Original closed-form solution performance without bias}
...
Without sensor bias, the original closed-form robustly estimate all the properties (below $0.05\%$ error) after $2$ seconds of integration.
...
we corrupt the accelerometer measurements by an artificial bias (Fig. \ref{fig:biasAccCF}).
...
Since our cost function requires an initialization and is non-convex (see Fig. \label{fig:cost}),
the optimization process can be stuck in local minima.
However, We can initialize the optimization process with $B = 0_3$ since the bias is usually rather small.
...
\begin{figure}
  ...
  \caption{Gyroscope bias estimation from nonlinear minimization of the residual.
    ...
    The true bias is $B=-[0.0584, 0.0751, 0.0308]$ with magnitude $|B|=0.1$.\label{fig:gBiasEstimate}}
\end{figure}
...
This method can robustly estimate high values of the gyroscope bias \ref{fig:gBiasEstimate}.
...
However, for very short time of integration ($<1$ seconds), the proposed method can fail by providing a gyroscope bias much larger than the correct one.
...
%% ***TODO HERE:****
The value of $\lambda$ should be set with respect to the variation of the gyroscope bias.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.489\columnwidth}
    ...
    \caption{Artificial $|bias|=0.05$.\label{fig:validBias1}}
  \end{subfigure}
  \begin{subfigure}[b]{0.489\columnwidth}
    \resizebox{\columnwidth}{!}{\input{graph/results2.8s-CF3-CF3Opt-Bias0.1.tex}}
    \caption{Artificial $|bias|=0.1$.\label{fig:validBias2}}
  \end{subfigure}
...
we corrupt the gyroscope measurements with an artificial bias in Fig. \ref{fig:validBias1} and Fig. \ref{fig:validBias2}.
